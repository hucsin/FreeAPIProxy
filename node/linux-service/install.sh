#!/bin/sh
# ============================================================================
#  freeapi —— 一键把 Node.js 出口代理安装为 Linux systemd 常驻服务（开机自启）
#
...
#    /etc/systemd/system/freeapi.service  systemd 单元（服务名即 freeapi）
...
if ! id freeapi >/dev/null 2>&1; then
  ...
  useradd --system --no-create-home --shell "${NOLOGIN}" freeapi
...
if [ -f "${ENV_FILE}" ]; then
  ...
  # 只用 LC_ALL=C + tr 取字母数字，避免 base64 换行 / 特殊字符污染 sed 替换
