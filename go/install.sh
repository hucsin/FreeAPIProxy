...
fetch_and_build() {
    ...
    ( cd "$tmpdir" && CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags "-s -w" -o go-proxy go-proxy.go )
...
[Unit]
Description=FreeApiGo (freeapi) reverse proxy
